-- pavlo.sql
CREATE TYPE stan AS ENUM ('Sprawny', 'Nie sprawny');

CREATE TABLE stacje
(
    id    SERIAL PRIMARY KEY,
    nazwa VARCHAR(100) NOT NULL,
    szerokosc NUMERIC(8, 5) NOT NULL,
    dlugosc NUMERIC(8, 5) NOT NULL,
    UNIQUE(szerokosc, dlugosc)
);
...
(
    ...
    stacja1_id                    INTEGER REFERENCES stacje (id) NOT NULL,
    tor1                          INTEGER,
...
(
    ...
    klasa        INTEGER NOT NULL CHECK(1 <= klasa and klasa <= 2)
);
...
(
    ...
    klient_id INT REFERENCES klient(id),
...
(
    id       SERIAL PRIMARY KEY,
    wysokosc INTEGER NOT NULL CHECK (wysokosc BETWEEN 0 AND 100)
);
...
(
    ...
    czas_od_poczatku              INTERVAL  NOT NULL
);
...
CREATE TABLE historia_lokomotywy(
    id_lokomotywy INT REFERENCES lokomotywy(id),
    stan stan,
    data timestamp NOT NULL,
    PRIMARY KEY(id_lokomotywy, data)
);

CREATE TABLE historia_wagonu(
    id_wagonu INT REFERENCES wagony(id),
    stan stan,
    data timestamp NOT NULL,
    PRIMARY KEY(id_wagonu, data)
);